...
class AiAnalysis extends Page implements HasForms, HasActions
{
    ...
    public function submit(): void
    {
        ...
        try {
            ...
            $prompt = "Available tables and columns (YOU MUST STRICTLY USE ONLY THESE):\n$schemaContext\n\nUser Instruction: $command\n\nTechnical Instruction: Generate the SQL query to fetch this data. CRITICAL RULES:\n1. USE ONLY the tables and columns listed above. Do NOT invent or guess columns that are not in the list.\n2. Verify the column exists in the EXACT table you are querying.\n3. Return the first column as the text label (e.g. name, title, category) and subsequent column(s) as numeric value(s).\n4. DO NOT use WHERE filters unless the user explicitly asks for them, to avoid returning empty results.\n5. DO NOT use GROUP BY if the user just asks for a list (e.g. 'products with highest prices', just use ORDER BY and LIMIT 10).\n6. Translate all returned text labels (like month names) to '{$idiomaStr}'.";
